...
namespace JsonHtml
{
    public partial class Form1 : Form
    {
        public Form1()
        {
            ...
            StreamWriter writer = new StreamWriter("c:/test/students.json");
            writer.Write(s);
            writer.Close();


            writer = new StreamWriter("c:/test/teachers.json");
            writer.Write(t);
            writer.Close();

            StreamReader reader = new StreamReader("c:/test/students.json");
            s = reader.ReadToEnd();
            reader.Close();

            reader = new StreamReader("c:/test/teachers.json");
            t = reader.ReadToEnd();
            reader.Close();
